...
function FeaturesSection() {
  ...
  const features = ["bmsDashboard", "aiAnomaly", "revenueEngine", "safetySystem"];
...
function StatsRow() {
  const stats = [
    { value: "1min", label: "데이터 수집 주기" },
    { value: "99.7%", label: "시스템 가동률" },
    { value: "0건", label: "안전 사고" },
    { value: "96.2%", label: "평균 SoH" },
  ];
...
function ScenariosSection() {
  const t = useTranslations("ess.monitoring.scenarios");
...
function TechSpecSection() {
  const t = useTranslations("ess.monitoring.techSpec");

  const specs = [
    { label: "BMS 게이트웨이", value: t("bms") },
    { label: "프로토콜", value: t("protocol") },
    { label: "수집 주기", value: t("interval") },
    { label: "호환 배터리", value: t("batteries") },
...
export default function ESSMonitoringPage() {
  const t = useTranslations("ess.monitoring");
  ...
  return (
    <>
      ...
      <section className="pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-xs text-primary font-medium tracking-[0.2em] uppercase">ESS Intelligence</p>
