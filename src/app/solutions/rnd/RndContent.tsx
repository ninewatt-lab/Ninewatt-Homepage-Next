...
function ProjectRow({ project }: { project: RndProject }) {
          ...
          <td colSpan={4} className="px-4 py-5">
            <div className="grid gap-4 md:grid-cols-2 text-sm">
              ...
              <div className="flex flex-wrap gap-x-8 gap-y-2 md:col-span-2 pt-2 border-t border-border/50">
                <div>
                  <span className="text-xs text-muted">사업유형</span>
                  <p className="text-foreground/90">{project.detail!.category}</p>
                </div>
                <div>
                  <span className="text-xs text-muted">주관부처</span>
                  <p className="text-foreground/90">{project.detail!.department}</p>
                </div>
                <div>
                  <span className="text-xs text-muted">총사업규모</span>
...
function ProjectTable({ projects }: { projects: RndProject[] }) {
  return (
      ...
      <section className="border-b border-border px-6 pb-16 pt-16">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-lg text-muted">
            정부 R&D 과제 수행을 통한 기술력 확보 — 총 {rndProjects.length}건 (수행중{" "}
            {inProgress.length}, 완료 {completed.length})
          </p>
          <p className="mt-2 text-sm text-muted">
            과제명을 클릭하면 상세 내용을 확인할 수 있습니다.
          </p>
        </div>
      </section>
