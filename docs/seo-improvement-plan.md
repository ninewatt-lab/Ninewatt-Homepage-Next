...
| §7 클라이언트 렌더링 전환 | 중간 | **치명적** | **P3 → P0 승격** |
...
1. **§7의 우선순위가 완전히 뒤바뀐다.** GPTBot·ClaudeBot·PerplexityBot은 Googlebot과 달리 **JavaScript를 거의 실행하지 않는다.** 구글에게 `"use client"` 페이지는 "색인이 늦는" 문제지만, AI 엔진에게는 **아예 존재하지 않는 페이지**다. SEO 관점의 P3가 GEO 관점에서는 P0다.
